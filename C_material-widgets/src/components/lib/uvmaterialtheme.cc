...
CUVMaterialThemePrivate::~CUVMaterialThemePrivate() = default;

QColor CUVMaterialThemePrivate::rgba(int r, int g, int b, qreal a) { // NOLINT
	QColor color(r, g, b);
	color.setAlphaF(a);
	return color;
}
...
CUVMaterialTheme::~CUVMaterialTheme() = default;

QColor CUVMaterialTheme::getColor(const QString& key) const {
	Q_D(const CUVMaterialTheme);
...
void CUVMaterialTheme::setColor(const QString& key, const QColor& color) {
	Q_D(CUVMaterialTheme);

	d->colors.insert(key, color);
}

void CUVMaterialTheme::setColor(const QString& key, const Material::Color color) {
	Q_D(CUVMaterialTheme);
...
QIcon CUVMaterialTheme::icon(const QString& category, const QString& icon) {
