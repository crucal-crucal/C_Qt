 ...
 *  \internal
 */
CUVMaterialSliderStateMachine::CUVMaterialSliderStateMachine(CUVMaterialSlider* slider, CUVMaterialSliderThumb* thumb, CUVMaterialSliderTrack* track)
: QStateMachine(slider), m_slider(slider), m_thumb(thumb), m_track(track), m_topState(new QState(QState::ParallelStates)),
  m_fstState(new QState(m_topState)), m_sndState(new QState(m_topState)), m_inactiveState(new QState(m_fstState)),
  m_focusState(new QState(m_fstState)), m_slidingState(new QState(m_fstState)), m_pulseOutState(new QState(m_focusState)),
  m_pulseInState(new QState(m_focusState)), m_minState(new QState(m_sndState)), m_normalState(new QState(m_sndState)) {
	addState(m_topState);
	...
	auto animation = new QPropertyAnimation(thumb, "haloSize", this);
	...
	QAbstractTransition* transition = new QEventTransition(slider, QEvent::FocusIn);
	...
	customTransition = new CUVMaterialStateTransition(SliderNoFocusMouseLeave);
	...
	// Slider pressed
	transition = new QSignalTransition(slider, &CUVMaterialSlider::sliderPressed);
	...
	// Slider released
	transition = new QSignalTransition(slider, &CUVMaterialSlider::sliderReleased);
	transition->setTargetState(m_focusState);
	animation = new QPropertyAnimation(thumb, "diameter", this);
	animation->setDuration(70);
	transition->addAnimation(animation);
	...
	customTransition = new CUVMaterialStateTransition(SliderChangedFromMinimum);
	...
	customTransition = new CUVMaterialStateTransition(SliderChangedToMinimum);
...
CUVMaterialSliderStateMachine::~CUVMaterialSliderStateMachine() = default;

void CUVMaterialSliderStateMachine::setupProperties() const {
 ...
 *  \internal
 */
CUVMaterialSliderThumb::CUVMaterialSliderThumb(CUVMaterialSlider* slider)
: CUVMaterialOverlayWidget(slider->parentWidget()), m_slider(slider), m_diameter(11),
  m_borderWidth(2), m_haloSize(0), m_offset(0) {
...
void CUVMaterialSliderThumb::paintEvent(QPaintEvent* event) {
	...
	const QPointF disp = Qt::Horizontal == m_slider->orientation()
	...
	const QRectF geometry = Qt::Horizontal == m_slider->orientation()
		                        ? QRectF(m_offset, m_slider->height() / 2 - CUV_MATERIAL_SLIDER_MARGIN, // NOLINT
		                                 CUV_MATERIAL_SLIDER_MARGIN * 2, CUV_MATERIAL_SLIDER_MARGIN * 2).translated(m_slider->pos())
		                        : QRectF(m_slider->width() / 2 - CUV_MATERIAL_SLIDER_MARGIN, m_offset, // NOLINT
		                                 CUV_MATERIAL_SLIDER_MARGIN * 2, CUV_MATERIAL_SLIDER_MARGIN * 2).translated(m_slider->pos());

	const qreal s = m_slider->isEnabled() ? m_diameter : 7;
 ...
 *  \internal
 */
CUVMaterialSliderTrack::CUVMaterialSliderTrack(CUVMaterialSliderThumb* thumb, CUVMaterialSlider* slider)
: CUVMaterialOverlayWidget(slider->parentWidget()), m_slider(slider), m_thumb(thumb), m_trackWidth(2) {
	slider->installEventFilter(this);

	setAttribute(Qt::WA_TransparentForMouseEvents, true);

	connect(slider, &CUVMaterialSlider::sliderMoved, this, [this]() {
		this->update();
	});
}
...
void CUVMaterialSliderTrack::paintEvent(QPaintEvent* event) {
	...
	if (Qt::Horizontal == m_slider->orientation()) {
		painter.translate(m_slider->x() + CUV_MATERIAL_SLIDER_MARGIN,
		                  m_slider->y() + m_slider->height() / 2 // NOLINT
		                  - static_cast<qreal>(m_trackWidth) / 2);
	} else {
		painter.translate(m_slider->x() + m_slider->width() / 2 // NOLINT
		                  - static_cast<qreal>(m_trackWidth) / 2,
		                  m_slider->y() + CUV_MATERIAL_SLIDER_MARGIN);
	}
