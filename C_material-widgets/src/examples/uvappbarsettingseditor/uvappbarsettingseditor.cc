...
void CUVAppBarSettingsEditor::createCtrl() {
	...
	m_pLbInbox->setAttribute(Qt::WA_TranslucentBackground);
	m_pLbInbox->setForegroundRole(QPalette::WindowText);
	m_pLbInbox->setContentsMargins(6, 0, 0, 0);

	QPalette palette = m_pLbInbox->palette();
	palette.setColor(m_pLbInbox->foregroundRole(), Qt::white);
	m_pLbInbox->setPalette(palette);
...
void CUVAppBarSettingsEditor::customLayout() const {
	...
	m_pMaterialAppBar->appBarLayout()->addWidget(m_pLbInbox);
