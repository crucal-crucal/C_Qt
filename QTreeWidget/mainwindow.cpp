...
//在鼠标位置获取按钮名字
// 实现QAction的enterEvent事件处理函数
void MainWindow::eventFilter(QEvent* event){
    if (event->type() == QEvent::Enter) { // 当鼠标移动到按钮上时D
        QAction* action = qobject_cast<QAction*>(this); // 将 this 转换为 QAction 对象
        if (action) {
            QString actionName = action ->toolTip(); // 获取按钮的名称
            QToolTip::showText(QCursor::pos(), actionName); // 在鼠标位置显示按钮名称
        }
    }
}

// 实现QAction的leaveEvent事件处理函数
void MainWindow::leaveEvent(QEvent* event) {
    if (event->type() == QEvent::Leave) { // 当鼠标移开QAction时
        ui->statusbar->showMessage(tr("节点标题"));
    }
}
...
//设置每个Action按钮的标签
void MainWindow::setactionToolTip()
{
    ui->actAddFolder->setToolTip(tr("选择目录，创建目录节点"));
    ui->actAddFiles->setToolTip(tr("选择多个文件，创建文件节点"));
    ui->actDeleteItem->setToolTip(tr("删除当前节点"));
    ui->actScanItems->setToolTip(tr("遍历所有节点，节点标题前加*"));
    ui->actDockFloat->setToolTip(tr("使图片目录树窗口浮动显示"));
    ui->actDockVisible->setToolTip(tr("显示或隐藏图片目录树"));
    ui->actZoomFitH->setToolTip(tr("图片高度适应显示区高度"));
    ui->actZoomFitW->setToolTip(tr("图片宽度适应显示区宽度"));
    ui->actZoomIn->setToolTip(tr("图片放大为原来的%120"));
    ui->actZoomOut->setToolTip(tr("图片缩小为原来的%80"));
    ui->actZoomRealSize->setToolTip(tr("图片已实际大小显示"));
    ui->exit->setToolTip(tr("退出本系统"));
}

//将所有按钮连接两个信号，一个信号为更新状态栏文本，另一个为恢复状态栏文本
void MainWindow::connectActions()
{
    connect(ui->actAddFolder, &QAction::hovered, this, &MainWindow::updateStatusbarText);
    connect(ui->actAddFiles, &QAction::hovered, this, &MainWindow::updateStatusbarText);
    connect(ui->actDeleteItem, &QAction::hovered, this, &MainWindow::updateStatusbarText);
    connect(ui->actScanItems, &QAction::hovered, this, &MainWindow::updateStatusbarText);
    connect(ui->actDockFloat, &QAction::hovered, this, &MainWindow::updateStatusbarText);
    connect(ui->actDockVisible, &QAction::hovered, this, &MainWindow::updateStatusbarText);
    connect(ui->actZoomFitH, &QAction::hovered, this, &MainWindow::updateStatusbarText);
    connect(ui->actZoomFitW, &QAction::hovered, this, &MainWindow::updateStatusbarText);
    connect(ui->actZoomIn, &QAction::hovered, this, &MainWindow::updateStatusbarText);
    connect(ui->actZoomOut, &QAction::hovered, this, &MainWindow::updateStatusbarText);
    connect(ui->actZoomRealSize, &QAction::hovered, this, &MainWindow::updateStatusbarText);
    connect(ui->exit, &QAction::hovered, this, &MainWindow::updateStatusbarText);

    connect(ui->actAddFolder, &QAction::triggered, this, &MainWindow::clearStatusbarText);
    connect(ui->actAddFiles, &QAction::triggered, this, &MainWindow::clearStatusbarText);
    connect(ui->actDeleteItem, &QAction::triggered, this, &MainWindow::clearStatusbarText);
    connect(ui->actScanItems, &QAction::triggered, this, &MainWindow::clearStatusbarText);
    connect(ui->actDockFloat, &QAction::triggered, this, &MainWindow::clearStatusbarText);
    connect(ui->actDockVisible, &QAction::triggered, this, &MainWindow::clearStatusbarText);
    connect(ui->actZoomFitH, &QAction::triggered, this, &MainWindow::clearStatusbarText);
    connect(ui->actZoomFitW, &QAction::triggered, this, &MainWindow::clearStatusbarText);
    connect(ui->actZoomIn, &QAction::triggered, this, &MainWindow::clearStatusbarText);
    connect(ui->actZoomOut, &QAction::triggered, this, &MainWindow::clearStatusbarText);
    connect(ui->actZoomRealSize, &QAction::triggered, this, &MainWindow::clearStatusbarText);
    connect(ui->exit, &QAction::triggered, this, &MainWindow::clearStatusbarText);
}
    ...
    , ui(new Ui::MainWindow)
{
    ui->setupUi(this);
    //创建状态栏上的组件
    labNodeText=new QLabel(tr("节点标题"), this);
    labNodeText->setMinimumWidth(200);
    ui->statusbar->addWidget(labNodeText);
//    setStatusBar(statusBar());
//    statusBar()->showMessage(tr("节点标题"));

    setactionToolTip();//为所有按钮设置提示

    connectActions();//将所有Action按钮与窗口底部状态栏相连，实现鼠标事件
...
void MainWindow::updateStatusbarText()
{
    ...
    if(action)
    {
        ui->statusbar->showMessage(action->toolTip());
    }
}
