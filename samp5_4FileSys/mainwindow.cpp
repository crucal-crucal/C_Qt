    ...
    ui(new Ui::MainWindow)
{
    ...
    root = m_model->index(QDir::currentPath());//保留当前根目录

    ui->treeView->setModel(m_model);  //设置数据模型
    ui->listView->setModel(m_model);  //设置数据模型
    ui->tableView->setModel(m_model); //设置数据模型

    connect(ui->actbackroot, &QAction::triggered, this, MainWindow::on_actbackroot_triggered);
...
void MainWindow::on_treeView_clicked(const QModelIndex &index)
{
    ...
    ui->listView->setRootIndex(index);//设置根目录
...
{
    QString dir = QFileDialog::getExistingDirectory(this, "选择目录", QDir::homePath());
    if (!dir.isEmpty())
    {
...
void MainWindow::on_actbackroot_triggered()
{
//    qDebug()<<1;
    ui->treeView->setRootIndex(root);
    ui->listView->setRootIndex(root);
    ui->tableView->setRootIndex(root);
}
