...
public:
	...
	/*
	 * @param: strFile 文件路径, stStreamInfo 推送流信息, szPlay 播放窗口大小,
	 * pairRecvCodecPara 接收的编码参数, mode 打开模式, bLoop 是否循环播放, bPicture 是否只播放图片
	 */
...
private:
	...
	CEncodeMuteAudioThread* m_pEncodeMuteThread{ nullptr };                  // 静音音频编码线程
	CPushThread* m_pPushThread{ nullptr };                                   // 推流线程
	CCodecThread::OpenMode m_eMode{ CCodecThread::OpenMode::OpenMode_Play }; // 打开模式，是播放还是推流
	bool m_bLoop{ false };
	bool m_bPicture{ false };
	eDecodeMode m_eDecodeMode{ eDecodeMode::eDecodeMode_CPU }; // 解码模式
