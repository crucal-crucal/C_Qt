...
QMyThreadPool::QMyThreadPool(QObject *parent) : QObject{parent}, QRunnable{} {
  // 当前线程执行完任务自动销毁
  this->setAutoDelete(true);
}

void QMyThreadPool::run() { qDebug() << "finish run to Pool"; }
...
//                     lock(this->mutex);//为其上锁，防止其他线程竞争，如果锁正在被使用则阻塞此线程。匿名函数执行完后自动释放锁
//                     this->cv.wait(lock, [this]() //等待通知并进行条件判断后
//                     决定是否向下执行。如果不满足条件则阻塞本线程
//                     {
//                         return !this->is_running || !this->tasks.empty();
//                         //如果线程池在运行或则任务队列有任务则继续向下执行
//                     });
//                     if (!this->is_running && this->tasks.empty()) return;
//                     //如果线程池结束运行并且任务队列没有任务了就终止该线程
//                     task =
//                     std::move(this->tasks.front());//向任务队列里取一个任务
//                     this->tasks.pop();//弹出任务
//                 }
//                 task();//运行任务
//             }
...
//     //将函数和其参数绑定在一起后给task,forward<> 类型转发函数模版
//     {
