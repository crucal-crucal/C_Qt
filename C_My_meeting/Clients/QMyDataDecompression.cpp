...
QMyDataDecompression::QMyDataDecompression(QObject *parent) : QObject{parent} {
  ...
  qDebug() << "----------------------------------------------------------------"
              "-----";
  connect(this, SIGNAL(finished_extractPictrueData(QByteArray)), this,
          SLOT(testPictrue(QByteArray)));
#endif
...
QByteArray QMyDataDecompression::changedPixmapToByteArray(QPixmap &map) {
  ...
  if (map.isNull())
    return QByteArray(); // 如果图像为空，做转换
  QPixmap map1 =
      map.scaled(QSize(400, 300), Qt::KeepAspectRatio, Qt::FastTransformation)
          .scaled(QSize(200, 150), Qt::KeepAspectRatio,
  ...
  map1.save(
      &buffer, "jpg",
      90); // 向buffer里写数据，90代表数据的强度，越高在传输图片时就会越清晰
...
QPixmap QMyDataDecompression::changedByteArrayToPixmap(QByteArray &byteArray) {
  ...
  map.scaled(QSize(300, 300), Qt::KeepAspectRatio,
             Qt::SmoothTransformation); // 缩放，按比例，图片平滑处理
...
void QMyDataDecompression::startExtractPacked(QByteArray &pack) {
  ...
  get_Data =
      (receiveData *)buffer
          .data(); //|强制将其转换为receiveData类型
                   //---无需手动释放空间,因为get_Data是(receiveData*)分配而来的空间|
...
QByteArray QMyDataDecompression::packVideoData(
    QByteArray &video_data, QByteArray &audio_data) // 打包要发送的视频数据
{
  /*brief
   * 此函数只能传视频或则音频，不能同时传视频和音频
   * 后期优化 -- 协议冲突
   * */
  ...
  video_audio_data.append(
      audio_data); // 定义一个video_audio_data，先放入视频数据，后放入音频数据，便于下一步拷贝
  temp = video_audio_data.data(); // 将视频音频数据先转换为char*
  memcpy(st_senddata.data, temp,
         video_data.length() +
             audio_data.length()); // 拷贝视频音频数据到数组里,
  // 第一个参数表示数据拷贝到的地方，第二个参数代表要拷贝的数据，第三参数代表要拷贝的长度
  /*******************************************************************************************************************/
  st_senddata.dataLenth =
      video_data.length() + audio_data.length(); // 数据区域长度
  st_senddata.check_sum =
      HEAD_LEN + video_data.length() + audio_data.length() +
      OTHER_LEN; // 校验和 = 数据长度 + 数据帧头长度 + 额外长度
  /*@brief
   * HEAD_LEN :帧头字节长度
   * 16       :另外4个int类型，一个int是4字节
   * */
  /*************************************************构造要发送的数据包***************************************************/
  sendTcpData.resize(HEAD_LEN + video_data.length() + audio_data.length() +
                     OTHER_LEN); // 开始构造数据包 -- 初始化一个数据包大小
  memcpy(sendTcpData.data(), &st_senddata,
         HEAD_LEN + st_senddata.dataLenth + OTHER_LEN -
             4); // 减掉校验和4字节，因为数据区可能会有空闲区域
  sendTcpData.append(
      st_senddata.check_sum); // 在尾巴上加上校验和 ---
                              // 避免数据区域和校验和之间有空闲区域
...
QByteArray QMyDataDecompression::packTextData(QString &text) {
  ...
  memcpy(st_senddata.data, temp,
         get_data.length()); // 不拷贝内存结构体时只会传一个指针过去
  st_senddata.dataLenth = get_data.length(); // 填入数据长度
  st_senddata.check_sum =
      get_data.length() + OTHER_LEN +
      HEAD_LEN; // 校验和 = 数据长 + 帧头(10字节) + 其他四个int -- 16字节
  /****************************************************构造要发送的数据包************************************************/
  sendTcpData.resize(
      (get_data.length() + OTHER_LEN - 4 +
       HEAD_LEN)); // 开始构造数据包 -- 不要加校验和 最后加在尾巴上
  memcpy(
      sendTcpData.data(), &st_senddata,
      (get_data.length() + OTHER_LEN - 4 +
       HEAD_LEN)); // 将数据拷贝到数据包sendTcpData，减掉校验和4字节，因为数据区可能会有空闲区域
...
QByteArray QMyDataDecompression::packPictrueData(QByteArray &pictrue_data) {
  ...
  memcpy(st_senddata.data, temp,
         pictrue_data.length()); // 不拷贝内存结构体时只会传一个指针过去
  st_senddata.dataLenth = pictrue_data.length(); // 填入数据长度
  st_senddata.check_sum =
      pictrue_data.length() + OTHER_LEN +
      HEAD_LEN; // 校验和 = 数据长 + 帧头(10字节) + 其他四个int -- 16字节
  /***********************************************构造要发送的数据包********************************************************/
  sendTcpData.resize(
      (pictrue_data.length() + OTHER_LEN - 4 +
       HEAD_LEN)); // 开始构造数据包 -- 不要加校验和 最后加在尾巴上
  memcpy(
      sendTcpData.data(), &st_senddata,
      (pictrue_data.length() + OTHER_LEN - 4 +
...
void QMyDataDecompression::extractVideoData(receiveData *target_data) {
  ...
  // target_wrap #字节存储排列:帧头(10字节) + 数据长度(4字节) + 视频长度(4字节)
  // + 音频长度(4字节) + 数据 + 校验和(4字节)
  /**************************************解析帧头(10字节) + 数据长度(4字节) +
   * 图片长度(4字节) + 音频长度(4字节)********************/
  //==================================================2.视频数据0x02=========================================================
...
QByteArray QMyDataDecompression::changedPixmapToByteArray(QPixmap& map)
{
    ...
    QPixmap map1 = map.scaled(QSize(400,300),Qt::KeepAspectRatio,Qt::FastTransformation).scaled(QSize(200,150),Qt::KeepAspectRatio,Qt::SmoothTransformation);
    QByteArray byteArray = QByteArray();//初始化QByteArray
    QBuffer buffer;//缓冲区
    buffer.open(QIODevice::ReadWrite);//读写模式
    map1.save(&buffer, "jpg", 90); //向buffer里写数据，90代表数据的强度，越高在传输图片时就会越清晰
    byteArray.append(buffer.data());//转变为字节
    return byteArray;//返回处理后的字节数据
}
QPixmap QMyDataDecompression::changedByteArrayToPixmap(QByteArray& byteArray)
