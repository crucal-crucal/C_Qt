...
QMyShareDesktop::QMyShareDesktop(QObject *parent) : QObject{parent} {
  ...
  connect(this->timer, &QTimer::timeout, this,
          &QMyShareDesktop::GetDesktopPictrue); // 定时捕捉
}
...
void QMyShareDesktop::GetDesktopPictrue() {
  ...
  if (ans == 0) {
    ans++;
    qDebug() << "GetDesktopPictrue 所处线程:" << QThread::currentThread()
  ...
  QByteArray byteArray = QMyDataDecompression::changedPixmapToByteArray(
      map); // 静态成员函数，无需实例化即可调用
