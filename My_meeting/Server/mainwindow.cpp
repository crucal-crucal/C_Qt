...
MainWindow::MainWindow(QWidget *parent): QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    connect(ui->btn_Monitor, &QPushButton::clicked, this, &MainWindow::Listening_port); //监听端口按钮
...
//        this->showAudiotoUI(array);
        myDataDecompression->startExtractPacked(array); //处理来自客户端的数据包，目前会发出三个信号，连接到处理函数即可
        qDebug()<<"size::\t"<<array.size();
    });
    connect(myDataDecompression, &QMyDataDecompression::finished_extractPictrueData, this, &MainWindow::showPicture); //摄像头
    connect(myDataDecompression, &QMyDataDecompression::finished_extractTextData, this, &MainWindow::showText); //文本
    connect(myDataDecompression, &QMyDataDecompression::finished_extractVideoData, this, &MainWindow::showVideo); //音视频
}
...
void MainWindow::showAudiotoUI(QByteArray array)
{
    ...
    audioSink1->write(array.constData(), array.size());//将文件写入audioSink，用于播放音频
}
//监听端口
...
//显示图像
void MainWindow::showPicture(QByteArray array)
{
    this->showPicturetoUI(array);
}
//显示文本
