...
void MainWindow::on_act_openfile_triggered()
{
    ...
    if(!fileName.isEmpty())
    {
        ui->act_sendfile->setEnabled(true);
        ui->textEdit->appendPlainText(QString("打开文件 %1 成功！！").arg(fileName));
    }
}


void MainWindow::on_act_sendfile_triggered()
{
    ui->act_sendfile->setEnabled(false);
    bytesWritten = 0;
