	...
	switch (const auto msg = static_cast<MSG*>(message); msg->message) {
		...
		case WM_GETMINMAXINFO: {
			if (::IsZoomed(msg->hwnd)) {
				...
				frames.setLeft(static_cast<int>(lround(abs(frame.left) / dpr + 0.5)));
				frames.setTop(static_cast<int>(lround(abs(frame.bottom) / dpr + 0.5)));
				frames.setRight(static_cast<int>(lround(abs(frame.right) / dpr + 0.5)));
				frames.setBottom(static_cast<int>(lround(abs(frame.bottom) / dpr + 0.5)));
...
void CUVBaseDialog::addTitleButton(QAbstractButton* pButton, const TitleButtonRole emButtonRole) {
