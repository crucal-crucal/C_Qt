...
FramelessWidget::FramelessWidget(QWidget* parent) : QWidget(parent) {
	...
	::SetWindowLong(hwnd, GWL_STYLE, static_cast<LONG>(style | WS_MAXIMIZEBOX | WS_THICKFRAME | WS_CAPTION));
#endif
}
...
void FramelessWidget::doResizeEvent(QEvent* event) { // NOLINT
...
#endif
{
	if (eventType == "windows_generic_MSG") {
		...
		if (const auto msg = static_cast<MSG*>(message); msg->message == WM_NCCALCSIZE) {
			*result = 0;
			return true;
		} else if (msg->message == WM_SYSKEYDOWN) { // NOLINT
			//屏蔽alt键按下
...
#endif
	} else if (eventType == "NSEvent") { // NOLINT
#ifdef Q_OS_MACOS
#endif
...
void FramelessWidget::setPadding(const int _padding) {
	this->padding = _padding;
}

void FramelessWidget::setMoveEnable(const bool _moveEnable) {
	this->moveEnable = _moveEnable;
}

void FramelessWidget::setResizeEnable(const bool _resizeEnable) {
