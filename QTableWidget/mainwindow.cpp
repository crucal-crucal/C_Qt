...
{
    ui->chkBoxTabEditable->setChecked(1);
    ui->chkBoxRowColor->setChecked(1);
    ui->chkBoxHeaderH->setChecked(1);
    ui->chkBoxHeaderV->setChecked(1);
    ui->rBtnSelectItem->setChecked(1);
}
    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->tableInfo->setColumnCount(6);      //设置表格列数
...
{
    //    QString str;
    QTableWidgetItem    *item = ui->tableInfo->item(0, 0);

    if(item!=nullptr)
    {
        //qDebug()<<ui->tableInfo->rowCount();
        //qDebug()<<ui->tableInfo->columnCount();
        ui->plainTextEdit->clear();  //文本编辑器清空
        for (int i=0;i<ui->tableInfo->rowCount();i++)   //逐行处理
        {
            QString str = QString("%1 %2 %3 ").arg("第").arg(i+1).arg("行");
    ...
    {
        ui->plainTextEdit->appendPlainText(tr("你小子别用空表！！！"));
    }
}
